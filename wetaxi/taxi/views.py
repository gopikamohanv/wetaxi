...
@login_required
@user_passes_test(taxi_check)
def taxi_cancel_enquiry(request, pk):
	response = {}
	booking = get_object_or_404(TaxiBookingSchedule, pk=pk)
...
@login_required
@user_passes_test(taxi_check)
def taxi_confirm_booking(request, pk):
	response = {}
	booking = get_object_or_404(TaxiBookingSchedule, pk=pk)

	advance = 0
	if 'advance' in request.POST and request.POST['advance']:
		advance = float(request.POST['advance'])

	if 'total' in request.POST and request.POST['total']:
		total = float(request.POST['total'])
	else:
		return HttpResponse(
			json.dumps(
					{'status':False, 'message':'Please enter total amount!!'}
			),
			content_type = 'application/json'
		)

	BookingPayment.objects.create(booking=booking, advance_payment=advance, total_payment=total)
	...
	return HttpResponse(
		json.dumps(
				{'status':True, 'message':'Booking Confirmed Successfully!!'}
		),
		content_type = 'application/json'
	)
